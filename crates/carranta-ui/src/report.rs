...
/// being signed out.
/// `active` names the lit pill, `history`, `corpus` or `account`, or
/// nothing; a page that is none of them leaves them all quiet, which is
/// itself an answer to where you are.
pub(crate) fn masthead_as(
    context: &str,
    links: &[(&str, &str)],
    active: &str,
    who: &crate::home::Who,
...
pub(crate) fn masthead_home(links: &[(&str, &str)], who: &crate::home::Who) -> String {
    mast("<h1 class=\"mark\">Carranta</h1>", "", links, "", who)
}
...
fn mast(
    ...
    if who.signed_in {
        ...
        b.push_str("<a href=\"/signin\">Sign in</a>");
    }
    b.push_str("</nav>");
    b.push_str("</div>");
    b.push_str("</header>");
...
pub fn page(saved: &Saved, study: &Study, viewer: &crate::home::Who) -> String {
    let r = &study.report;
...
.headTabs { display: flex; gap: 4px; background: var(--card);
            border: 1px solid var(--border); border-radius: var(--radius-md);
            padding: 3px; }
.headTabs a { font: 600 13px Figtree, system-ui, sans-serif; text-decoration: none;
              color: var(--muted-foreground); padding: .35em .85em;
              border-radius: calc(var(--radius-md) - 2px); }
.headTabs a:hover { color: var(--foreground); }
.headTabs a.on { background: var(--primary); color: var(--primary-foreground); }
...
/* The activity weeks, shared by the account's own graph and the corpus's
   all-games one: columns of days, Monday first, an orange ramp. */
...
/* The candle on a per-turn column: the whisker is the standard deviation
   either side of the mean, drawn up the middle of the column's bar. */
.whisk { position: absolute; left: 50%; width: 2px; margin-left: -1px;
...
mod tests {
    ...
    fn one_header_sits_above_every_screen_in_the_application() {
        ...
        let home = masthead_home(&[("/lobby", "New game")], &nobody_who());
    ...
    fn the_page_says_what_the_game_did() {
        ...
        let html = page(&history[1], &s, &nobody_who());
    ...
    fn the_first_game_says_it_has_nothing_to_compare_with() {
        ...
        let html = page(&only, &s, &nobody_who());
