...
- Any per-player aggregate must decide how to treat substituted games. **Default: flag them and exclude from rated statistics**, since neither the departed human nor the substituting bot played a whole game.
  - **Overruled once the server was live** (see `ui.md` §12). Excluding makes leaving free, so the correct play when losing is to close the tab, and it throws away the game of everyone who stayed. What ships: a finished game is rated whenever *somebody* was still at the table for it, with anyone who walked out ranked where they finished; only a game the bots finished on everybody's behalf is dropped. The exclusion default stands for statistics *about play* (what a seat did with its turns), where a bot's turns really are not that person's; it does not stand for the result. Walkouts are instead reported as their own count, beside the rating rather than inside it (`ui.md` §12).
...
|---|---|---|---|
| ART-1 | **Port distribution and placement**: how many 3:1 and 2:1 ports, which resource each 2:1 serves, and which intersection pairs carry them | Both setup modes | **Arrangement built**: 4× 3:1 and 5× 2:1 (one per resource), each on an adjacent pair of coastal intersections, with a generic between every two specific ports. Placement is the most even one available: the coast bunches at the island's six corners, so spacing counted in intersections is uneven to look at, and the arrangement chosen is the one an exhaustive search finds tightest by angle (a 10° spread between widest and narrowest, which 9 ports on a six-cornered coast cannot beat). Which resource each 2:1 serves relative to the terrain under it is still open |
| ART-2 | **Number-disc placement order** for Random Setup, the traversal that R-3.3 walks | Random Setup | **Built.** The path spirals in: the coast counterclockwise from a corner, then the inner ring, then the middle. A connected walk, every step landing on a hex touching the one before. The starting corner is drawn at random from the six, which is the only variety the placement has once the sequence is fixed. The eighteen disc values are content rather than rule (`DISCS` in `state.rs`), ordered so that following the path keeps the 6s and 8s apart; the desert can still shift everything after it, which is what D-6's check is for |
...
| D-6 | Red numbers (6/8) adjacent on a randomly generated board | **Game option**, defaulting to the constraint enabled. **Built**, as the default: the engine deals the discs again until no two reds touch, which keeps the result uniform over the boards that satisfy the constraint where repairing in place would bias it. 86% of unconstrained deals violate it, measured over 20 000 boards, so this is not a rare corner. The *toggle* is not exposed yet, it belongs to lobby configuration (§6.2), so today every board is generated with the constraint on. | R-3.12 |
